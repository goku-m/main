...
func (r *TodoRepository) CreateTodo(ctx context.Context, payload *todo.CreateTodoPayload) (*todo.Todo, error) {
	stmt := `
		INSERT INTO
			todos (
				title,
	...
	if err != nil {
		return nil, fmt.Errorf("failed to execute create todo query for  title=%s: %w", payload.Title, err)
	}

	todoItem, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[todo.Todo])
	if err != nil {
		return nil, fmt.Errorf("failed to collect row from table:todos for  title=%s: %w", payload.Title, err)
	}

	return &todoItem, nil
}

func (r *TodoRepository) GetTodoByID(ctx context.Context, todoID uuid.UUID) (*todo.Todo, error) {
	stmt := `
	SELECT
		u.*
...
`
	...
	if err != nil {
		return nil, fmt.Errorf("failed to execute get todo by id query for todo_id=%s: %w", todoID.String(), err)
	}

	todoItem, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[todo.Todo])
	if err != nil {
		return nil, fmt.Errorf("failed to collect row from table:todos for todo_id=%s todo_id=%s: %w", todoID.String(), err)
	}

	return &todoItem, nil
}

func (r *TodoRepository) CheckTodoExists(ctx context.Context, todoID uuid.UUID) (*todo.Todo, error) {
	stmt := `
		SELECT
			*
		FROM
	...
	if err != nil {
		return nil, fmt.Errorf("failed to check if todo exists for todo_id=%s: %w", todoID.String(), err)
	}
...
func (r *TodoRepository) UpdateTodo(ctx context.Context, payload *todo.UpdateTodoPayload) (*todo.Todo, error) {
